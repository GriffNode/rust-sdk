...
pub async fn create_billing_checkout(configuration: &configuration::Configuration, create_billing_checkout_request: models::CreateBillingCheckoutRequest) -> Result<models::TransactionEnvelope, Error<CreateBillingCheckoutError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/billing/checkout", local_var_configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.json(&create_billing_checkout_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateBillingCheckoutError> = serde_json::from_str(&local_var_content).ok();
